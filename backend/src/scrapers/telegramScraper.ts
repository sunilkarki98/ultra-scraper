...
export class TelegramScraper extends BaseScraper<UniversalScrapeResult> {
    ...
    protected async scrape(options: ScrapeOptions): Promise<UniversalScrapeResult> {
        ...
        try {
            ...
            logger.warn("Telegram page header not found, might be a private channel or invalid URL.");
        }
    ...
    private async extractPosts(page: Page) {
        return page.evaluate(() => {
            ...
            postElements.forEach((postEl) => {
                ...
                if (photoLink) {
                    const bgStyle = (photoLink as HTMLElement).style.backgroundImage;
                    if (bgStyle) {
