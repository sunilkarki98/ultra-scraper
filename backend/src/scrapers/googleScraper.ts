...
import { ScrapeOptions, ScrapeResult as UniversalScrapeResult } from './universalScraper';
import { logger } from '../utils/logger';
import { ProxyService } from '../common/proxy/proxy.service';
import { Page } from 'playwright';

export class GoogleScraper extends BaseScraper<UniversalScrapeResult> {
    constructor(proxyService?: ProxyService) {
        super(proxyService);
    }

    protected async scrape(options: ScrapeOptions): Promise<UniversalScrapeResult> {
        ...
        try {
            ...
            logger.warn("Google search results selector not found, might be a captcha or different layout.");
        }
        ...
        return {
            ...
            leads: {
                emails: [],
                phones: localPackResults.map(r => r.phone).filter(p => p !== null) as string[],
                socialLinks: []
            },
            jsonLd: []
        };
    ...
    private async handleConsent(page: Page) {
        try {
    ...
    private async extractLocalPack(page: Page) {
        return page.evaluate(() => {
            const businesses: { name: string; rating: string; address: string; phone: string | null; website: string | null }[] = [];
